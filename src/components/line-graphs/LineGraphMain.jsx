...
const getGraphData = state => {
  const { data } = state.measurementReducer;
...
const LineGraphMain = () => {
  ...
  const times = [];
  const { data, currentlySelected } = useSelector(getGraphData);
  ...
  const reformatData = (datas, color) => {
    const tempData = {
      data: [],
      borderWidth: 1,
      borderColor: color,
      backgroundColor: color,
      label: datas[0].metric,
      fill: false,
      pointRadius: 0,
      yAxisID: datas[0].unit,
    };
    datas.map(item => {
